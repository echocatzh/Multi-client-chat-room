...
def keyGeneration(keyLength: int,seed = -1):
    if seed != -1:
        random.seed(seed)
...
def str_decryption(sentence, d, n):
    str_list = []
    for t in sentence:
        str_list.append(decryption(ord(t), d, n))
    return str(str_list)


def str_encryption(sentence, e, n):
    str_list = []
    for t in sentence:
        str_list.append(encryption(ord(t), e, n))
    return str_list


def numlist_to_decryption_str(nums, d, n):
    s = ""
    for i in nums:
        a = decryption(i, d, n)
        s += chr(a)
